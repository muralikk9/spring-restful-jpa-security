...
public class EmployeeController {
	...
	public ResponseEntity<List<Employee>> getAllEmployees(@AuthenticationPrincipal final UserDetails userDetails) {
		List<Employee> employees = empService.getAll();
		if (employees.isEmpty()) {
			logger.debug("Employees does not exists");
			return new ResponseEntity<List<Employee>>(HttpStatus.NO_CONTENT);
		}
		logger.debug("requested by " + userDetails.getUsername() + " has roles" + userDetails.getAuthorities().toString());
